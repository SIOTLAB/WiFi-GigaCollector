...
# Retrieve ath9k transmission queue statistics per Wi-Fi Access Category (AC) from driver using debugfs then publish on ZMQ socket
    ...
    with open(path, 'r') as f:
        ...
        while not stop_event.is_set():
            try:
                ...
                std_out = " ".join(list(output))

                # Use regex to ensure proper format before processing
                match = re.findall(Q_MATCH, std_out)
                ...
                if match:
                    ...
                    pub_dict = {f'{process_name}_ts': ts,
                                ...
                                'vo_qdepth_val':  int(match[VO][QDEPTH_VAL]),
                                'vo_adepth_val':  int(match[VO][ADEPTH_VAL]),
                                'vo_pending_val': int(match[VO][PENDING_VAL]),

                                'vi_qdepth_val':  int(match[VI][QDEPTH_VAL]),
                                'vi_adepth_val':  int(match[VI][ADEPTH_VAL]),
                                'vi_pending_val': int(match[VI][PENDING_VAL]),

                                'be_qdepth_val':  int(match[BE][QDEPTH_VAL]),
                                'be_adepth_val':  int(match[BE][ADEPTH_VAL]),
                                'be_pending_val': int(match[BE][PENDING_VAL]),

                                'bk_qdepth_val':  int(match[BK][QDEPTH_VAL]),
                                'bk_adepth_val':  int(match[BK][ADEPTH_VAL]),
                                'bk_pending_val': int(match[BK][PENDING_VAL]),
...
if __name__ == '__main__':
    ...
    cfg_d['dc_pub_bind_addr'] = 'tcp://localhost:5651' # ath9k queue statistics DS (this) publisher socket address to bind
