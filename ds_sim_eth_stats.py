...
# Randomly simulate Ethernet WAN statistics using ioctl ethtool client from pyroute2 library then publish on ZMQ socket

def sim_eth_stats(cfg_d): # Config dictionary
    ...
    while not stop_event.is_set():
        try:
            ...
            # Randomly simulate values
            rx_packets = random.randint(0, 1000)
            tx_packets = random.randint(0, 1000)
            rx_bytes = random.randint(0, 100000)
            tx_bytes = random.randint(0, 100000)

            dt_rx_packets = max(0, rx_packets - prev_rx_packets)
            dt_tx_packets = max(0, tx_packets - prev_tx_packets)
            dt_rx_bytes = max(0, rx_bytes - prev_rx_bytes)
            dt_tx_bytes = max(0, tx_bytes - prev_tx_bytes)
            dt_ts = ts - prev_ts
...
if __name__ == '__main__':
    cfg_d = {}
    cfg_d['process_name'] = 'DS_SimEthStats'
    # Make sure the protocols match between publishers and subscribers
    cfg_d['pub_bind_addr'] = 'tcp://localhost:5652' # Ethernet WAN statistics DS (this) publisher socket address to bind
